...
const [newData,setNewData] =useState({
  ...
  const fetchBlog =async () => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_LOCAL_PORT}/api/blogs/single/${slug}`,
          { cache: "no-store" }
      );
  ...
  try {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_LOCAL_PORT}/api/blogs/${slug}`,
      {
        method: "PUT",
        body: formData, 
      }
    ...
    router.push(`/admin/blogs/view/${slug}`);
  } catch (err) {
    console.error(err);
            ...
            <div className="flex gap-3">
              {isViewMode ? (
                ...
                <>
                  <button
                    type="button"
                    onClick={() => router.push(`/admin/blogs/view/${slug}`)}
                    className={buttonClasses.secondary}
                  >
